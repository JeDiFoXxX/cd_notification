...
### Описание проекта
`checkdev_notification` — это микросервис уведомлений, 
отвечающий за рассылку сообщений пользователям через различные каналы:

* Telegram-бот 
* внутренняя система сообщений
* подписки на темы и категории
* 
Сервис обеспечивает централизованную доставку уведомлений о событиях 
(интервью, отклики, утверждения, отказы, обновления) 
другим участникам платформы CheckDev. Он тесно интегрируется с другими микросервисами экосистемы 
— такими как **checkdev_auth, checkdev_desc**, **checkdev_mock**, **checkdev_generator** 
— и взаимодействует с ними через **REST** и **Eureka**.
